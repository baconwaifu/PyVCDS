...
common = VWCanBus(None, {
    0x280: lambda a: "ECU 1 RPM: {}".format(struct.unpack(">H", data[4:6])[0] // 4),
    0x300: lambda a: "Diag Outbound 1",
    0x301: lambda a: "Diag Outbound 2",
    0x302: lambda a: "Diag Outbound 3", #these can go all the way up to 30F, but those aren't common unless you're doing bulk live-data collection
...
models = {
 "3C": VWCanBus(common, { 
    ...
    0x0d0: lambda a: "Power Steering Torque: {} Steering Angle Segment: {}".format(a.data[2] * 0.00390625, a.data[4] * 0.175781),
    0x181: lambda a: "Window Controls (Tentative)",
    0x1a0: lambda a: "ABS Vehicle Speed: {}".format(struct.unpack(">h", data[2:4])[0]),
    ...
    0x531: lambda a: "Light Control", #note: this has a pair of sequence bytes following the state frame. *be careful*.
