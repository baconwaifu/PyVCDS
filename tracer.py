...
def kwp_decode(buf):
    if buf[0] & 0x40 == 0x40:  # response, harder to decode.
        if buf[0] == 0x58:
            ...
            print(
                "Response to Module DTCs:")  # NOTE: the response format is identical, but the actual *response* is different.
            count = buf[1]
            print("NOTE: proper OBD notation is not yet implemented. raw hex codes are displayed.")
            print("NOTE: leading character is (usually) implied by the module being read")
            if not count == 0:
        ...
        if op == 0xA8:  # disconnect
            ...
            if op & 0x10 == 0x10:
                if self.inlen != len(self.inbuf):
                    util.log(3, "WARN: frame length mismatch! expected {}, got {}. Attempting to continue...".format(
                        self.inlen, len(self.inbuf)))
    ...
    def _recv(self, buf):
        ...
        if buf[0] == 0x7f:  # negative response
            ...
            if buf[0] - 0x40 in reqmap:
                ...
                util.log(3,
                         "Positive KWP response to Unknown service '{}' (OEM specific)\nReport trace to maintainer with VIN.".format(
                             buf[0] - 0x40))
