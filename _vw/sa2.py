...
class SA2BytecodeRunner:
    ...
    def run(self):
        if self.ran:
            # short-circuit if we've already derived the key; should never actually *hit*
            # this, but easy performance op if the key is re-used.
            return self.key
...
# used for CAN-bus VWs as well as old k-line vehicles. possibly some UDS ones as well?
# (source for K-line cars using it: http://www.freediag.org/opendiag/4983.html)
# gives access to security level 2, or "write"
# security level 4 is read-only access, and is a much simpler algorithm
# ((seed + prekey) & 0xffffffff) where prekey is probably(?) 0x00011170
class XorKey:
    # the "processing factor" for the seed; *should* be selected from ECUID,
    # but for some reason a lot use the last factor? (dec 63)
    SEED_DATA = [
    ...
    def run(self):
        seed = self.seed
        # note: original java implementation used an "unsigned shift" for the
        # rshift, but python numbers don't "have" a visible sign bit
        ...
        for i in range(5):  # 5 shifts
            if (seed & 0x80000000) == 0x80000000:  # if the "to overflow" bit is set, xor with 'key'
                seed = (self.SEED_DATA[self.ecu]) ^ ((seed << 1) | (seed >> 31)) & 0xffffffff
                # rotate left, xor, and clamp.
            else:
