...
def mod_menu(car):
    ...
    with mod as m:
        while True:
            op = menu.selector(["Read Module ID", "Read Manufacturer Info", "Read Firmware Version", "Read Coding",
                                "Re-Code module (EXPERIMENTAL)", "Read Measuring Block", "Load Labels for Module",
                                "Drop to Python Console", "Back"])
            if op == 0:
                try:
                    print(m.readID())
                except kwp.ENOENT:
            ...
            if op == 6:
                ...
                if m.pn in vw.labels:  # there's more to this behind the scenes; it will transparently attempt to load labels on the fly when called for
                    ...
                    print(
                        "Error loading labels: missing or encrypted label file")  # again, I *WILL NOT HELP* with decrypting clb labels.
            if op == 7:
                import pdb;
                pdb.set_trace()
            if op == 8:
...
def main(sock):
    with vwtp.VWTPStack(sock) as stack, vw.VWVehicle(stack) as car:  # host the connection outside the menu loop.
        while True:
            opt = ["Enumerate Modules", "Connect to Module", "Read DTCs by module",
                   "Read Measuring Data Block by module", "Long-Coding", "Load Labels from VCDS",
                   "Load Labels from JSON", "Back"]
